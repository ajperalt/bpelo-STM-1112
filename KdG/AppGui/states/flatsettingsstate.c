...
#define NAME_RECT	"flat"


void init_flatsettingsstate() {
	COMPONENT* lblTitle;
	COMPONENT* lblHeight;
	COMPONENT* lblWidth;
	COMPONENT* spnHeight;
	...
	rectangle = createRectangle(NAME_RECT, 0, componentGetBottom(lblTitle) + 5,
	...
	width = componentGetWidth(rectangle);

	componentSetX(rectangle, 20);
	componentSetX(lblHeight, componentGetRight(rectangle) + 5);
	componentSetX(spnHeight, componentGetRight(lblHeight) + 5);
	componentSetY(lblHeight, componentGetBottom(rectangle) - componentGetHeight(
			rectangle) / 2 - componentGetHeight(lblHeight) / 2);
	componentSetY(spnHeight, componentGetBottom(rectangle) - componentGetHeight(
			rectangle) / 2 - componentGetHeight(spnHeight) / 2);
	
	componentSetX(lblWidth, componentGetX(rectangle));
	componentSetY(lblWidth, componentGetBottom(rectangle)+5);
	componentSetX(spnWidth, componentGetRight(lblWidth) +5);
	componentSetY(spnWidth, componentGetBottom(rectangle)+5);
	componentSetY(btnOk, componentGetBottom(spnWidth)+5);
	componentSetY(btnBack, componentGetBottom(btnOk)+5);
}

void draw_flatsettingsstate() {
...
int handle_flatsettingsstate() {
	...
	while (stop == 0) {
		...
		if (keysGetDown() == 1) {
			...
			break;
			case 2: // measure
				settings.type = MECHANIC_FLAT;
				settings.flat.width = spinnerGetValue(buttons[1]);
				settings.flat.height = spinnerGetValue(buttons[0]);
